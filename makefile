...
# COMPONENTS+=CYW920735Q60EVB-01_MS_design_modus
# INCLUDES+=$(CY_APP_PATH)/COMPONENT_CYW920735Q60EVB-01_MS_design_modus/GeneratedSource
#endif
...
ifeq ($(TARGET), CYW920820REF-MS-01)
 CY_APP_DEFINES += -DMOUSE_PLATFORM
 CY_APP_DEFINES += -DCLICK_USE_KEYSCAN
 CY_APP_DEFINES += -DUSE_COMBO_PAIRING
 CY_APP_DEFINES += -DMOTION_SUPPORT
 CY_APP_DEFINES += -DCLICK_SUPPORT
 CY_APP_DEFINES += -DSCROLL_SUPPORT
else
 CY_APP_DEFINES += -DMOTION_SUPPORT
 CY_APP_DEFINES += -DCLICK_SUPPORT
 CY_APP_DEFINES += -DCLICK_USE_BUTTON       #Use Buttons for click
 CY_APP_DEFINES += -DUSE_LSM9DS1
endif
...
ifeq ($(hci_dump), 1)
 ...
 ifeq ($(TESTING_USING_HCI),1)
  ...
  # key report is for Client Control test only
  CY_APP_DEFINES += -DKEY_REPORT_SUPPORT
 endif
endif
